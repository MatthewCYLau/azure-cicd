...
# Notebook 1: Ingestion & Schema Enforcement
...
# Notebook 2: Financial Aggregations & Risk Metrics
...
resource "databricks_job" "risk_pnl_pipeline" {
  ...
  task {
    ...
    # Declares task dependency: Gold runs ONLY after Silver succeeds
    depends_on {
      task_key = "bronze_to_silver_cleaning"
    }
  ...
  schedule {
    quartz_cron_expression = "0 0 2 * * ?" # 02:00 AM UTC
    timezone_id            = "UTC"
  }
}
